...
function allManagedTrails(){
 ...
 baseTrails.forEach(t=>{if(builtInOverrides[t.id])records.set(t.id,{...t,...builtInOverrides[t.id],id:t.id,source:'Local edit'})});
 customTrails.forEach(t=>records.set(t.id,{...t,source:'Local edit'}));
...
async function loadSharedData(){
 try{
  ...
  const loaded=(trailRows||[]).map(row=>row.data).filter(t=>t&&t.id);
...
async function persistVote(id,value,observations){
 const cleanObservations=Array.isArray(observations)?observations:[];
 try{
  if(!value&&!cleanObservations.length){
   await supabaseRpc('remove_my_trail_report',{p_trail_id:id,p_reporter_token:reporterToken()});
   localStorage.removeItem('trailVote:'+id);
  }else{
   await supabaseRpc('save_my_trail_report',{
    p_trail_id:id,p_reporter_token:reporterToken(),
    p_recommendation:value||null,p_observations:cleanObservations
   });
   localStorage.setItem('trailVote:'+id,JSON.stringify({value:value||null,observations:cleanObservations,time:Date.now()}));
  }
  await refreshCommunityReports();
  render();reopenReport(id);
 }catch(error){
  console.error('Could not save the shared trail report',error);
  alert('Your report could not be saved. Please check your connection and try again.');
 }
}
