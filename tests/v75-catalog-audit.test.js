...
assert.strictEqual(trails.length, 148, 'V75 catalog must contain 148 active systems');
assert.deepStrictEqual(Object.fromEntries(['OH','WV','IN','PA','MI'].map(code=>[code,trails.filter(t=>(t.stateCode||'OH')===code).length])),{OH:37,WV:16,IN:26,PA:28,MI:41});
...
assert(html.includes('<span>v75.1</span>'),'Visible V75.1 version missing');
